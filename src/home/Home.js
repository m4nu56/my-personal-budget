...
function fetchMovements () {
    makeFetch(`movements`).then(response => {
        this.setState({
...
function fetchCategories () {
    makeFetch(`categories`).then(response => {
        this.setState({
            lstCategories: response
        });
    }).catch(error => {
        this.addNotification(`Erreur lors de la récupération des catégories. ${error}`, 'error');
    });
}
                    ...
                    <div>
                        ...
                        <div className="container" style={{marginTop: '30px'}}>
                            <div className="row align-items-center">
                                <div className="col">
                                    <Route path="/analyze" exact={true} render={props => <BudgetAnalyze {...props}
                                                                                                        lstMouvement={this.state.lstMouvement}
                                                                                                        lstCategories={this.state.lstCategories}/>}/>
                                    <Route
                                        path="/mouvement"
                                        exact={true}
                                        render={props => (
                                    ...
                                    <Switch>
                                        ...
                                        />
                                        <Route path="/mouvement/:id" render={props => <BudgetForm {...props}
                                                                                                  onSaveMouvement={this.onSaveMouvement}
                                                                                                  lstCategories={this.state.lstCategories}/>}/>
                                    </Switch>
